"""
Y-Triples RDF graph serializer for RDFLib.
See XXX for details about the format.
"""
...
__all__ = ['YTSerializer']


class YTSerializer(Serializer):
    """
    ...
    def serialize(self, stream, base=None, encoding=None, **args):
        if base is not None:
            warnings.warn("YTSerializer does not support base.")
        if encoding is not None:
            warnings.warn("YTSerializer does not use custom encoding.")
        encoding = self.encoding
        for triple in self.store:
            stream.write(_yt_row(triple).encode(encoding, "replace"))
        stream.write(b("\n"))
...
def _yt_row(triple):
    if isinstance(triple[2], Literal):
        ...
        return u"%s %s %s mmgh?\n" % (triple[2].n3(),
                                  triple[0].n3(),
                                  _xmlcharref_encode(triple[1].n3()))
